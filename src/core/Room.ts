...
interface IRoom {
  ...
  createPipeTransport: (
    options: types.PipeTransportOptions,
...
export default class Room implements IRoom {
  ...
  public webrtcTransports: Map<string, types.WebRtcTransport<types.AppData>> =
    new Map();
  ...
  getAllConsumers(): Map<string, types.Consumer<types.AppData>> {
    ...
    this.participants.forEach((participant) => {
      participant.consumers.forEach((consumer, id) =>
        allConsumers.set(id, consumer),
      );
    });
    return allConsumers;
  }
  ...
  monitorHealth(): boolean {
    ...
    const hasActivity =
      this.participants.size > 0 || this.webrtcTransports.size > 0;
