import {
  ...
  createWorker as createMediaSoupWorker,
} from 'mediasoup';
...
class WorkerManager implements IWorkerManager {
    ...
    try {
      ...
      worker.on('died', () => {
        ...
        this.createWorker().catch((err) =>
          this.log(`Failed to create replacement worker: ${err}`),
        );
      });
  ...
  public removeWorker(workerId: string): void {
    ...
    if (worker) {
      ...
      this.log(
        `Worker ${workerId} removed. Active workers: ${this.workers.size}`,
      );
    }
  }
  ...
  public cleanupIdleWorkers(idleThresholdMs: number = 600000): void {
    ...
    for (const [id, worker] of this.workers.entries()) {
      if (
        worker.getLastActivityTime() &&
        now - worker.getLastActivityTime()! > idleThresholdMs
      ) {
        this.log(
